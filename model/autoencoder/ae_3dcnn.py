...
class MotionEncoderC3D(nn.Module):
    def __init__(self, out_dim=256, block_depth=3, latent_dim=256):
        super(MotionEncoderC3D, self).__init__()
    ...
    def forward(self, input_frames, mode='conv'):
        if input_frames.size(2) == 3:
            input_frames = torch.transpose(input_frames, 1, 2)
        code = self.conv_block(input_frames)  # (batch, 256, 16, 8, 8)
...
class Decoder(nn.Module):
    def __init__(self, block_depth=3, mode='interpolate'):
        ...
        if mode == 'interpolate':
            self.deconv_block = nn.Sequential(DeconvUpsamplingBlock3D(in_channel=256, depth=block_depth, channels=256),
                                              DeconvUpsamplingBlock3D(in_channel=256, depth=block_depth, channels=128),
                                              DeconvUpsamplingBlock3D(in_channel=128, depth=block_depth, channels=64),
                                              DeconvUpsamplingBlock3D(in_channel=64, depth=block_depth, channels=32))
        elif mode == 'transpose':
            self.deconv_block = nn.Sequential(DeConvBlock3D(in_channel=256, depth=block_depth, channels=256),
                                              DeConvBlock3D(in_channel=256, depth=block_depth, channels=128),
                                              DeConvBlock3D(in_channel=128, depth=block_depth, channels=64),
                                              DeConvBlock3D(in_channel=64, depth=block_depth, channels=32))
        self.colorize = ConvBlock3D(in_channel=32, depth=1, channel=3, skip=False)
        pass
    ...
    def forward(self, code, mode='conv'):
        ...
        if reconstruction.size(1) == 3:
            reconstruction = torch.transpose(reconstruction, 1, 2)
