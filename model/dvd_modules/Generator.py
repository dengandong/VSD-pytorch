...
from model.dvd_modules.GResBlock import GResBlock
from model.dvd_modules.Normalization import SpectralNorm
from model.ConvGRU import ConvGRU
from model.Attention import SelfAttention, SeparableAttn
...
class Generator(nn.Module):
    ...
    def __init__(self, in_dim=120, latent_dim=8, n_class=4, ch=32, n_frames=48, hierar_flag=False):
        super().__init__()
    ...
    def forward(self, x, class_id):
        ...
        if self.hierar_flag is True:
            noise_emb = torch.split(x, self.in_dim, dim=1)  # batch, 120
        else:
            noise_emb = x
        ...
        for k, conv in enumerate(self.conv):
            if isinstance(conv, ConvGRU):
                ...
                layer_out_list.append(y[:, :, 0, :, :].squeeze())
                # print(y[:, :, 0, :, :].squeeze().size())
                # print(y.size())
                B, T, C, W, H = y.size()
                y = y.view(-1, C, W, H)
                ...
                y = conv(y, condition)  # BT, C, W, H  # this condition is the second input of ConditionNorm
                _, C, W, H = y.size()
                layer_out_list.append(y.view(-1, T, C, W, H)[:, :, 0, :, :].squeeze())
                # print(y.view(-1, T, C, W, H)[:, :, 0, :, :].squeeze().size())
